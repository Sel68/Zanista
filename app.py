...
from preprocessing import (read_file, generate_eda_report, handle_missing_values,
                         remove_outliers, scale_by_factor,
                         change_column_type, get_descriptive_stats)
...
if 'model_trained_features' not in st.session_state: # To store features used for training
...
with st.sidebar:
    ...
    uploaded_file = st.file_uploader("Upload Data File", type=['csv', 'json', 'xlsx', 'xls'])
    ...
    if uploaded_file:
        if st.session_state.get('uploaded_filename') != uploaded_file.name:
            ...
            try:
                ...
                st.rerun() # Use rerun to reload the app state cleanly
...
if st.session_state.get('df') is not None:
    # Make sure df is always the one from session state
    ...
    with tabs[0]:
        ...
        if df is not None:
             ...
             num_rows = st.number_input("Rows to show:", min_value=1, value=st.session_state['num_rows_preview'], key='rows_preview')
             if num_rows != st.session_state['num_rows_preview']:
                  st.session_state['num_rows_preview'] = num_rows # Update state
                  st.rerun() # Rerun if value changes

             st.subheader("Head")
             st.dataframe(df.head(st.session_state['num_rows_preview']), use_container_width=True)

             st.subheader("Tail")
             st.dataframe(df.tail(st.session_state['num_rows_preview']), use_container_width=True)
    ...
    with tabs[1]:
        ...
        st.info("Note: Preprocessing steps modify the current dataset used for visualization and modeling.")
        ...
        with st.expander("EDA Report"):
            if st.button("Generate", key='eda_btn'):
                try:
                    ...
                    st.dataframe(pd.DataFrame.from_dict(report['column_analysis'], orient='index'), use_container_width=True)
                except Exception as e:
        ...
        with st.expander("Handle Missing Values"):
            ...
            if not missing_cols:
                ...
                st.dataframe(missing_summary[missing_summary > 0].reset_index(name="Count"), use_container_width=True)
                col_to_handle = st.selectbox("Column:", missing_cols, key='miss_col', index=0) # Default to first
                if col_to_handle:
                    method = st.radio("Method:", ('Fill', 'Drop Rows'), key=f'miss_method_{col_to_handle}', horizontal=True)
                    fill_value = None
                    if method == 'Fill':
                        ...
                        if pd.api.types.is_numeric_dtype(col_type):
                             ...
                             fill_value = st.number_input("Fill with:", value=default_fill if pd.notna(default_fill) else 0.0, key=f'miss_fill_{col_to_handle}', format="%g")
                        else:
                             # Use mode, handle cases where mode is empty or non-unique
                             mode_val = df[col_to_handle].mode()
                             default_fill = mode_val.iloc[0] if not mode_val.empty else ''
                             fill_value = st.text_input("Fill with:", value=default_fill, key=f'miss_fill_{col_to_handle}')
                    ...
                    if st.button("Apply Treatment", key=f'miss_apply_{col_to_handle}'):
                        ...
                        if method == 'Drop Rows':
                            ...
                            strat[col_to_handle] = {'method': 'fill', 'value': fill_value}

                        try:
                            df_processed, rows_dropped = handle_missing_values(st.session_state['df'], strat)
                            st.session_state['df'] = df_processed # Update the main dataframe
                            st.success(f"Missing values in '{col_to_handle}' handled using '{method}'. Rows dropped: {rows_dropped if method == 'Drop Rows' else 0}.")
        ...
        with st.expander("Change Data Type"):
             ...
             if selected_col:
                 ...
                 target_types = ['Numeric (Float)', 'Numeric (Integer)', 'Text (String)', 'Category', 'Date/Time']
                 new_type = st.selectbox("New type:", target_types, key='dtype_new')
                 if st.button(f"Convert", key='dtype_apply'):
                      try:
                           df_processed, failed = change_column_type(st.session_state['df'], selected_col, new_type)
                           st.session_state['df'] = df_processed
                           st.success(f"Converted '{selected_col}'. New type: {df_processed[selected_col].dtype}.")
                           if failed > 0: st.warning(f"{failed} values could not be converted to the target numeric/date type and became NaN.")
                           # Clear model if data changes
                           st.session_state['model'] = None
                           st.session_state['model_trained_features'] = None
                           st.rerun()
        ...
        with st.expander("Remove Outliers (Z-Score)"):
            ...
            if numeric_cols:
                cols_outlier = st.multiselect("Columns to check:", numeric_cols, key='outlier_cols')
                threshold = st.number_input("Z-Score Threshold:", min_value=1.0, value=3.0, step=0.1, key='outlier_thresh')
                if cols_outlier and st.button("Remove Rows", key='outlier_apply'):
                     try:
                         df_processed, removed = remove_outliers(st.session_state['df'], cols_outlier, threshold)
                         st.session_state['df'] = df_processed
                         st.success(f"Removed {removed} rows containing outliers based on Z-score > {threshold} in selected columns.")
                         # Clear model if data changes
                         st.session_state['model'] = None
                         st.session_state['model_trained_features'] = None
                         st.rerun()
        ...
        with st.expander("Remove Columns"):
            cols_remove = st.multiselect("Columns to remove:", df.columns.tolist(), key='remove_cols')
        ...
        with st.expander("Scale Data (Multiply)"):
             ...
             if numeric_cols_s:
                 cols_scale = st.multiselect("Columns to scale:", numeric_cols_s, key='scale_cols')
                 factor = st.number_input("Scaling Factor (k):", value=1.0, format="%g", key='scale_factor')
                 if cols_scale and st.button("Apply Scaling", key='scale_apply'):
                      try:
                          df_processed = scale_by_factor(st.session_state['df'], cols_scale, factor)
                          st.session_state['df'] = df_processed
    ...
    with tabs[2]:
        ...
        with st.expander("Descriptive Stats & Box Plot", expanded=True):
            numeric_cols = df.select_dtypes(include=np.number).columns.tolist()
            if numeric_cols:
                stat_col = st.selectbox("Select Numeric Column:", numeric_cols, key='stat_col')
                if stat_col:
                    ...
                    if st.checkbox("Show Box Plot", value=True, key='whisker_show'):
                        try:
                            ...
                            if fig:
                                ...
                                st.warning("Could not generate box plot (column might not be suitable).")
                        except Exception as e:
        ...
        with st.expander("Create Plots", expanded=True):
            ...
            if viz_type == "Pie Chart":
                ...
                x_options = [c for c in all_cols_options if c == "" or not pd.api.types.is_numeric_dtype(df.get(c))] # Usually categorical for pie
                ...
                y_disabled = True
                color_options = [c for c in all_cols_options if c == "" or not pd.api.types.is_numeric_dtype(df.get(c))] # Color usually by category
            elif viz_type == "Scatter Plot":
                x_options = numeric_cols_plot # Scatter needs numeric X
                y_options = numeric_cols_plot # Scatter needs numeric Y
            elif viz_type == "Bar Chart":
                x_options = [c for c in all_cols_options if c == "" or not pd.api.types.is_numeric_dtype(df.get(c))] # Bar usually categorical X
                y_label = "Y-axis (Optional Numeric for Mean):" # Y can be count or mean

            # Create columns for selectors
            c1, c2, c3 = st.columns(3)
            with c1: x_col = st.selectbox(x_label, x_options, key='viz_x')
            with c2: y_col = st.selectbox(y_label, y_options, key='viz_y', disabled=y_disabled)
            with c3: color_col = st.selectbox(color_label, color_options, key='viz_color', disabled=(viz_type == "Pie Chart"))
            ...
            if st.button("Generate Plot", key='gen_plot'):
                 ...
                 if not x_col:
                     ...
                     plot_possible = False
                      ...
                      plot_possible = False
                 elif viz_type == "Histogram" and x_col and not pd.api.types.is_numeric_dtype(df.get(x_col)):
                 ...
                 if plot_possible:
                      try:
                           ...
                           if fig:
                               ...
                               st.warning("Could not generate plot (check selections and data).")
                      except Exception as e:
    ...
    with tabs[3]:
        ...
        with st.expander("Train Model", expanded=True):
            ...
            if df is not None and not df.empty:
                 potential_targets = [col for col in df.columns if not pd.api.types.is_datetime64_any_dtype(df[col])] # Exclude datetime targets
                 if not potential_targets:
                     st.warning("No suitable target columns found in the current dataset (datetime columns excluded).")
                 else:
                     ml_target = st.selectbox("Target Variable (Y):", [""] + potential_targets, key='ml_target_sel', index=0) # Default to empty

                     model_options = []
                     task_type = None
                     if ml_target:
                        # Determine task type based on selected target in current df
                        target_series = df[ml_target].dropna() # Use dropna to handle potential missings during nunique check
                        if pd.api.types.is_numeric_dtype(target_series) and target_series.nunique() > 15:
                            ...
                            st.info("Detected Task: Regression (numeric target with >15 unique values)")
                        elif target_series.nunique() > 1 : # Classification if non-numeric or numeric with <=15 unique values
                            task_type = 'classification'
                            model_options = ["Decision Tree Classifier"] # Add more classifiers if needed
                            st.info("Detected Task: Classification (non-numeric target or <=15 unique numeric values)")
                        else:
                            st.warning("Selected target column has only one unique value after dropping NAs. Cannot train a model.")


                     if model_options: # Only show model selection if task is identified
                          ml_model_type = st.selectbox("Model Type:", model_options, key='ml_model_sel')
                          if st.button("Train Model", key='ml_train_btn'):
                              # Train using the current state of df
                              try:
                                   model, score, trained_task, training_features = train_model(st.session_state['df'], ml_target, ml_model_type)
                                   st.session_state['model'] = model
                                   st.session_state['model_score'] = score
                                   st.session_state['model_task'] = trained_task
                                   st.session_state['ml_target_trained'] = ml_target # Store trained target name
                                   st.session_state['model_trained_features'] = training_features # Store features used for training input form

                                   score_metric = "R2 Score" if trained_task == 'regression' else "Accuracy"
                                   st.success(f"Model trained successfully! Task: {trained_task}, {score_metric}: {score:.3f}")
                                   st.balloons()
                              except Exception as e:
                     ...
                     elif ml_target and not task_type: # If target selected but task not identified (e.g., only 1 unique value)
        ...
        if st.session_state.get('model') is not None and st.session_state.get('model_trained_features') is not None:
            st.markdown("---")
            ...
            if features_for_input and current_df_for_defaults is not None:
                with st.form(key='pred_form'):
                    ...
                    num_rows_grid = (len(features_for_input) + cols_per_row - 1) // cols_per_row
                    ...
                    for i, feature in enumerate(features_for_input):
                        ...
                        if feature in current_df_for_defaults.columns:
                            ...
                            if pd.api.types.is_numeric_dtype(dtype):
                                ...
                                input_val = float(default_val) if pd.notna(default_val) else 0.0
                                ...
                                options = sorted([str(v) for v in feat_series.dropna().unique()])
                                # Use mode of the current data as default
                                default_mode = feat_series.mode()
                                default_val = default_mode.iloc[0] if not default_mode.empty else (options[0] if options else "")
                                default_val_str = str(default_val)

                                # Find index, handle case where default might not be in options (e.g., after filtering)
                                try:
                                    default_index = options.index(default_val_str) if default_val_str in options else 0
                                except ValueError:
                                ...
                                pred_inputs[feature] = col_widget.text_input(f"{feature}", key=f'pred_{feature}')
                        else:
                            ...
                            col_widget.text_input(f"{feature} (Missing?)", value="", key=f'pred_{feature}', disabled=True)
                            pred_inputs[feature] = None # Mark as missing for later handling if needed
                    ...
                    if submitted:
                        ...
                        # Important: Ensure the prediction DataFrame columns match the order of features_for_input
                        pred_df = pred_df[features_for_input]

                        # Attempt basic type conversion based on the *current* dataframe's dtypes
                        # Might not be perfect but helps align types before encoding/prediction
                        for col in features_for_input:
                            if col in current_df_for_defaults.columns:
                                target_dtype = current_df_for_defaults[col].dtype
                                try:
                                ...
                                except Exception: # Broad except for conversion issues
                                        st.warning(f"Could not reliably convert input for '{col}'. Using raw input.")
                        ...
                        try:
                            ...
                            st.error("Ensure input values are appropriate for the model.")

            else:
                st.warning("Cannot create prediction interface. Model features or current data state missing.")
